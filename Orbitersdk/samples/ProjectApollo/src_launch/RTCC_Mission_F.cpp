...
bool RTCC::CalculationMTP_F(int fcn, LPVOID &pad, char * upString, char * upDesc, char * upMessage)
{
	...
	case 15: //MCC-3
	{
		...
		if (scrubbed)
		{
			...
			AGCStateVectorUpdate(uplinkdata, RTCC_MPT_CSM, RTCC_MPT_CSM, sv_ephem);
	...
	case 134: //TEI-32 UPDATE
	{
		...
		{
			strcpy(manname, "TEI-22");
			sv2 = coast(sv1, 5.5*2.0*3600.0);
		}
		else if (fcn == 35)
		{
			strcpy(manname, "TEI-23");
			sv2 = coast(sv1, 0.5*2.0*3600.0);
		}
		else if (fcn == 36)
		{
			strcpy(manname, "TEI-24");
			sv2 = coast(sv1, 0.5*2.0*3600.0);
		}
		else if (fcn == 37)
		{
			strcpy(manname, "TEI-25");
			sv2 = coast(sv1, 0.5*2.0*3600.0);
		}
		else if (fcn == 38)
		{
			strcpy(manname, "TEI-26");
			sv2 = coast(sv1, 0.5*2.0*3600.0);
		}
		else if (fcn == 39)
		{
			strcpy(manname, "TEI-27");
			sv2 = coast(sv1, 0.5*2.0*3600.0);
		}
		else if (fcn == 130)
		{
			strcpy(manname, "TEI-29");
			sv2 = coast(sv1, 2.5*2.0*3600.0);
		}
		else if (fcn == 131)
		{
		...
		if (fcn == 133)
		{
			...
			if (upString != NULL) {
				// give to mcc
				strncpy(upString, uplinkdata, 1024 * 3);
				strcpy(upDesc, "State vectors, target load");
			}
		}
	}
	break;
	...
	case 57: //REV 30 LANDMARK TRACKING PADs
	{
		...
		else if (fcn == 53 || fcn == 54 || fcn == 55 || fcn == 56)
		{
			...
			strcpy(form->LmkID[2], "F-1");
			opt.alt[2] = 0.0;
			opt.lat[2] = 1.6*RAD;
			opt.lng[2] = 86.88*RAD;

			strcpy(form->LmkID[3], "130");
	...
	case 60: //STATE VECTOR and LLS 2 REFSMMAT UPLINK
	{
		...
		if (upString != NULL) {
			// give to mcc
			strncpy(upString, uplinkdata, 1024 * 3);
			strcpy(upDesc, "CSM state vector, LLS2 REFSMMAT");
		}
	}
	break;
	...
	case 64: //LGC ACTIVATION UPDATE
	{
		...
		if (upString != NULL) {
			// give to mcc
			strncpy(upString, uplinkdata, 1024 * 3);
			strcpy(upDesc, "clock update, state vectors, LS REFSMMAT");
		}
	}
	break;
	...
	case 71: //DESCENT ORBIT INSERTION
	{
		...
		if (upString != NULL) {
			// give to mcc
			strncpy(upString, uplinkdata, 1024 * 3);
			strcpy(upDesc, "LM state vector, DOI target load");
		}
	}
	break;
	...
	case 76: //CSM BACKUP INSERTION UPDATE
	{
		...
		AP11ManeuverPAD(&opt, *form);
		strcpy(form->purpose, "Backup Insertion");
