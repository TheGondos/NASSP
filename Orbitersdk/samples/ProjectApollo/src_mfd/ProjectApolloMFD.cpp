...
static SOCKETHANDLE m_socket = OAPI_INVALID_SOCKET;				
static SOCKETHANDLE close_Socket = OAPI_INVALID_SOCKET;
...
void UplinkLMRTC(bool arm, bool set)
{
	if (g_Data.connStatus == 0) {
		...
		int port;
		if (g_Data.uplinkLEM > 0) { port = 14243; }
		else { port = 14242; }
		if (!oapiSocketConnect(m_socket, "127.0.0.1", port)) {
			g_Data.uplinkDataReady = 0;
			sprintf(debugWinsock, "FAILED TO CONNECT, ERROR");
...
void UplinkData()
{
	if (g_Data.connStatus == 0) {
		...
		int port;
		if(g_Data.uplinkLEM > 0){	port = 14243; }else{ port = 14242; }
		if (!oapiSocketConnect( m_socket, "127.0.0.1", port)) {
			g_Data.uplinkDataReady = 0;
...
// Constructor
ProjectApolloMFD::ProjectApolloMFD (int w, int h, VESSEL *vessel) : MFD2 (w, h, vessel)
...
bool ProjectApolloMFD::ConsumeKeyBuffered (int key) 
{
...
*/
	...
	if (!saturn && !lem) {
		skp->SetTextColor (oapiGetColour(255, 0, 0));
	...
	// Draw IMFD
		...
		skp->SetTextAlign(oapi::Sketchpad::LEFT);
		skp->SetTextColor (oapiGetColour(128, 128, 128));
		if (g_Data.iuVessel)
		{
...
// MFD message parser
OAPI_MSGTYPE ProjectApolloMFD::MsgProc (MFD_msg msg, MfdId mfd,  MFDMODEOPENSPEC *param, VESSEL *vessel)
{
	switch (msg) {
	case OAPI_MSG_MFD_OPENEDEX:
		// Our new MFD mode has been selected, so we create the MFD and
		// return a pointer to it.
		return (OAPI_MSGTYPE)(new ProjectApolloMFD (param->w, param->h, vessel));
	}
	return 0;
}
