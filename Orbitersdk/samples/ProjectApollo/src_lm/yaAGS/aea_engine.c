...
//
// I believe VC7 uses standard int64 formats, and not Microsoft's
...
#else
static const int64_t CONST64_1 = ~0377777777777i64;
static const int64_t CONST64_2 = 0177777777777i64;
static const int64_t CONST64_3 = 1i64;
#endif
...
GetLongAccumulator (ags_t *State)
{
  ...
  if (State->Accumulator & 0400000)	// Negative?
    lli |= CONST64_1;	// If so, then sign-extend.
  return (lli);
}

// Write a double-precision value to the A and Q registers.
static void
PutLongAccumulator (ags_t *State, int64_t lli)
{
...
aea_engine (ags_t * State)
{
  ...
  switch (OpCode)
    ...
    case 004:	// DVP
	  ...
	  //printf ("     %Lo %o\n", lli, i);
	  llk = ((uint64_t /* unsigned int64_t */) lli) / i;
	  if (0400000 & llk)
	    llk |= ~0777777;
	  ...
	  if (State->Accumulator != 0377777)
	    {
	      llk = ((uint64_t /* unsigned int64_t */) (lli * 2)) / i;
	      if (llk & 1)
